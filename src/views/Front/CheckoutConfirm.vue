        ...
        <div class="row justify-content-center bg-light my-3">
          <div class="col-md-8">
            <div class="d-flex py-3 align-items-center justify-content-center">
              ...
              <div class="px-5">
                <p class="h3">開幕歡慶</p>
                <p>輸入代號<span class="text-success fw-bold">  happybuy123  </span>即享九折優惠</p>
              </div>
            </div>
          </div>
        </div>
        <div class="row">
          ...
          <div class="line-info mb-3"></div>
            <div v-if="cart.length === 0 && !Loading">
              <p class="h4 mb-0">購物車沒有商品了，趕快去逛逛!<router-link to="/" class="btn btn-outline-dark ms-3">回首頁</router-link></p>
            </div>
            <div class="row py-3 align-items-center" v-for="item in cart" :key="item.id">
              ...
              <div class="col-8 col-md-5 d-flex flex-column justify-content-between">
                ...
                <div class="input-number-group input-group w-75">
                  <button type="button" class="btn btn-sm minus border border-dark btn-outline-dark" :disabled="loadingItem === item.id" @click="item.qty = minusQuantity(item.qty);changeProductQty(item)"><i class="bi bi-dash-lg"></i></button>
                  <input type="number" v-model.number="item.qty" :disabled="loadingItem === item.id" @change="item.qty = verifyNumber(item.qty);changeProductQty(item)" class="text-center form-control input-number h-100 rounded-0 border-dark">
                  <button type="button" class="btn btn-sm add border border-dark btn-outline-dark" :disabled="loadingItem === item.id" @click="item.qty = addQuantity(item.qty);changeProductQty(item)"><i class="bi bi-plus-lg"></i></button>
                </div>
              </div>
              <div class="col-8 col-md-3"><p class="h5 my-1"><span class="d-md-none d-inline-block">價格:</span>{{ $filters.currency(item.total) }}</p></div>
              <div class="col-4 col-md-2">
                <a href="#" @click.prevent="deleteProduct(item)"><i class="bi bi-x-square h4"></i></a>
              </div>
            </div>
            <hr>
            <div class="row">
              <div class="col-12 text-end">
                <p class="h4 py-2"><span class="me-3">總計:</span>{{ $filters.currency(total) }}</p>
                <p class="h4 py-2 text-success" v-if="total != final_total"><span class="me-3">優惠價:</span>{{ $filters.currency(final_total) }}</p>
              </div>
            </div>
            ...
            <div class="row" v-if="cart.length > 0">
              <div class="col-12 py-3 text-end btn-lg">
                <button class="btn btn-major" type="button" @click="confirmOrder"><span class="h5">下一步，填寫資料<i class="fas fa-feather-alt ms-2"></i></span></button>
              </div>
            </div>
          </div>
        </div>
