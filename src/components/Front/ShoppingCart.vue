  ...
  <transition name="slide">
    <div class="rightSidebar" v-if="openSidebar">
      <div class="ps-4 pe-3 py-3">
        ...
        <p class="text-center cart-title py-2 h5 fw-bold">購物車<i class="fas fa-shopping-cart ms-2"></i></p>
        <div v-if="!Loading && cart.length === 0 && cartVisible" class="text-center">購物車還沒有商品喔，<br>趕快新增一些吧!</div>
        ...
        <div class="d-flex justify-content-between align-items-center py-3" v-if="cart.length > 0 && cartVisible">
          <router-link class="btn btn-major" to="/checkoutConfirm" @click="openSidebar = false">結帳去<i class="fas fa-shopping-bag ms-2"></i></router-link>
          ...
          <div class="row py-3 border-bottom border-warning">
            ...
            <div class="col-9">
               <div class="input-number-group input-group">
                <button type="button" class="btn btn-sm minus border border-2" :disabled="loadingItem === item.id" @click="item.qty = minusQuantity(item.qty);changeProductQty(item)"><i class="bi bi-dash-lg"></i></button>
                <input type="number" v-model.number="item.qty" :disabled="loadingItem === item.id" @change="item.qty = verifyNumber(item.qty);changeProductQty(item)" class="text-center form-control input-number h-100 rounded-0">
                <button type="button" class="btn btn-sm add border border-2" :disabled="loadingItem === item.id" @click="item.qty = addQuantity(item.qty);changeProductQty(item)"><i class="bi bi-plus-lg"></i></button>
              </div>
            </div>
